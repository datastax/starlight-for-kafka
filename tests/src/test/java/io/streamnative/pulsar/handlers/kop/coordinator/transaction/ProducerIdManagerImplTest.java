...
public class ProducerIdManagerImplTest extends KopProtocolHandlerTestBase {
    ...
    protected void cleanZNode() throws Exception {
        Stat stat = mockZooKeeper.exists(ProducerIdManagerImpl.KOP_PID_BLOCK_ZNODE, null);
        if (stat != null) {
            mockZooKeeper.delete(ProducerIdManagerImpl.KOP_PID_BLOCK_ZNODE, -1);
        }
    }
    ...
    public void testExceedProducerIdLimit() throws Exception {
        mockZooKeeper.create(ProducerIdManagerImpl.KOP_PID_BLOCK_ZNODE, null, null, CreateMode.PERSISTENT);
        mockZooKeeper.setData(ProducerIdManagerImpl.KOP_PID_BLOCK_ZNODE,
                ProducerIdManagerImpl.generateProducerIdBlockJson(
                        new ProducerIdManagerImpl.ProducerIdBlock(
                                1, Long.MAX_VALUE - ProducerIdManagerImpl.PID_BLOCK_SIZE, Long.MAX_VALUE)), -1);
